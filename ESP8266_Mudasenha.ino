...
String pral = "<html>"
              ...
              "<input type='submit' value='Comprovar conexao' />"
              "</fieldset>"
              "</form>"
...
void wifi_conf() {
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    cuenta++;
    if (cuenta > 20) {
      graba(70, "noaconfigurado");
      server.send(200, "text/html", String("<h2>Nao realizou a conexion<br>dados não salvos.</h2>"));
      return;
    }
  }
  ...
  graba(1, getssid);
  graba(30, getpass);
  server.send(200, "text/html", String("<h2>Conexao realizada em: "
                                       + getssid + "<br> Pass ingresado: " + getpass + "<br>Datos correctamente salvos."));
...
void intento_conexion() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.print("Conexion bem sucedida a: ");
    Serial.println(ssid);
...
void setup() {
  Serial.begin(115200);
  EEPROM.begin(4096);
  WiFi.softAP("kwiek");      //Nombre que se mostrara en las redes wifi
